...
const createOrder = async (data) => {
  ...
  try {
    ...
    const inventoryProducts = await getInventoryByProductIds(
      client,
      productIds,
    );
...
const updateOrderStatus = async (data) => {
  ...
  try {
    ...
    if (data.status && data.status === "cancelled") {
      // if order is being cancelled, we need to release reserved inventory
      const existingOrder = await OrdersRepository.getOrderById(data.id);
      if (existingOrder) {
        await updateInventoryForProductIds(
          client,
          existingOrder.items,
          "release",
        );
    ...
    const order = await OrdersRepository.updateOrderStatus(
      client,
      data.id,
      data.status
    );
